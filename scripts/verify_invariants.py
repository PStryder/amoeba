...
MUTATIONS: list[Mutation] = [
    ...
    Mutation(
        ...
        "src/amoeba/mailbox.py",
        '                state = "unanswerable"',
        '                state = "answered"  # MUTANT: silence is a reply',
    ...
    Mutation(
        ...
        also=[("src/amoeba/reconstitution.py",
               '        if not held:\n            u.status = "unknown"\n',
               '        if not held:\n            u.status = "settled"  # MUTANT: unknown means removable\n'),
              ("src/amoeba/mailbox.py",
    ...
    Mutation(
        ...
        also=[("src/amoeba/homeostasis.py",
               ...
               "        told = False  # MUTANT: only the turn boundary tells the role"),
              ("src/amoeba/roles.py",
               "            if recorded and recorded != self.session_id and not self.current_turn_id:",
               "            if False:  # MUTANT: a role never heals"),
              ("src/amoeba/mailbox.py",
               '" WHERE role = ? AND session_handle = ? AND status != \'running\'"',
               '" WHERE role = ? AND ? IS NOT NULL AND status != \'running\'"')],
    ),
    ...
    Mutation(
        ...
        'Then wait',
        "emit a single tool call. Then wait",
        ["test_a_role_is_taught_the_syntax_its_parser_accepts"],
        layer="roles.ENVIRONMENT_BLOCK (what a role is told about invoking a capability)",
        note="The mutant is the block as it stood when no Ego or Id turn had ever executed a tool: it says to emit a call and never says what one looks like.",
    ),
    Mutation(
        "I114", "A role is not restarted for thinking",
        "src/amoeba/roles.py",
        '            self.inf_probe.call("health")',
        '            self.inf.call("health")  # MUTANT: probe through the busy client',
        ["test_health_answers_while_a_turn_holds_the_inference_client",
         "test_a_long_generation_does_not_get_its_role_restarted"],
        layer="RoleProcess.health (whether a liveness probe waits on a generation)",
        note="Defended at both layers, and the live one was checked against this mutant: its first version held the generation too briefly, passed with the defect present, and was lengthened until it reproduced the production restart exactly.",
    ),
    Mutation(
        "I115", "A malformed capability call is never delivered as an answer",
        "src/amoeba/roles.py",
        "            attempt = None if requests else malformed_call(said, self._offered())",
        "            attempt = None  # MUTANT: text that is not a request is a reply",
        ...
        layer="RoleProcess._turn / mailbox.assemble_answer (what counts as a reply)",
        note="Two anchors: the loop that should refuse the attempt and let the model retry, and the assembly that should withhold a piece a thought ended on. Either alone delivers board_post(author=...) to the operator, which is what happened live.",
        also=[("src/amoeba/mailbox.py",
               '        if isinstance(res, dict) and res.get("malformed_call"):',
               "        if False:  # MUTANT: deliver it anyway")],
    ),
    Mutation(
        "I117", "What a capability accepts is declared, and a refusal says so",
        "src/amoeba/role_env.py",
        '        if values:\n            entry["allowed"] = list(values)',
        '        if False:  # MUTANT: undeclared vocabulary\n            entry["allowed"] = list(values)',
        ["test_the_roles_are_told_the_values_they_are_checked_against",
         "test_a_refusal_says_what_would_have_been_accepted"],
        layer="role_env._schema / role_tool_invoke (what a model can learn about an argument)",
        note="Both halves of what Ego lacked live: the values up front, and the values after a miss. Separately verified to die for undeclared kinds and for a declared vocabulary that drifts from the constant the check reads.",
        also=[("src/amoeba/turn_api.py",
               '            if isinstance(details, dict) and details.get("allowed"):',
               "            if False:  # MUTANT: the refusal forgets what it wanted")],
    ),
    Mutation(
        "I118", "A session reads an unchanged declaration once",
        "src/amoeba/roles.py",
        "            if sha and held == (self.session_id, sha):",
        "            if False:  # MUTANT: re-send it every turn",
        ["test_a_session_reads_an_unchanged_declaration_once",
         "test_a_second_turn_does_not_pay_for_the_declaration_again"],
        layer="RoleProcess._turn (what a turn ingests about its environment)",
        note="The mutant is the old behaviour, which starved Id. Separately verified to die for a reference that crosses into a new session -- the case where skipping the declaration would leave a mind with none.",
    ),
    Mutation(
        "I119", "A role's arguments are checked before the verb sees them",
        "src/amoeba/turn_api.py",
        "        if problem:\n            return _recorded(",
        "        if False:  # MUTANT: dispatch whatever arrived\n            return _recorded(",
        ["test_a_wrong_type_is_refused_before_the_verb_runs"],
        layer="role_tool_invoke (what reaches a verb from a model)",
        note="The mutant is the old behaviour: the live reproducer's reason becomes AttributeError again. Separately verified to die for unchecked list items and for a checker that refuses what it cannot read.",
    ),
    Mutation(
        "I120", "A call that fails leaves nothing behind for the next one",
        "src/amoeba/rpc.py",
        "            except BaseException:\n                # A call that fails mid-flight",
        "            except ZeroDivisionError:  # MUTANT: keep the connection\n                # A call that fails mid-flight",
        ["test_a_late_reply_is_never_read_as_the_next_calls",
         "test_a_reply_to_another_request_is_refused"],
        layer="RpcClient.call (whose reply a call reads)",
        note="The mutant is the old behaviour: the second call fails on the timed-out socket, as the health probe did live. Separately verified to die for a reply with another request's id being believed.",
        also=[("src/amoeba/rpc.py",
               '            if resp.get("id") not in (None, req_id):',
               "            if False:  # MUTANT: believe any reply")],
    ),
    Mutation(
        "I121", "What the operator is shown is measured, and attributed",
        "src/amoeba/pulse.py",
        '                held = by_session.get(r.get("session_id")) or {}',
        "                held = r  # MUTANT: read context off health again",
        ...
        layer="SystemPulse._remote_facts / role_tool_invoke / prompt_incarnations (what the operator reads)",
        note="The primary mutant is the old pulse: nulls that looked like readings. Separately verified to die for unmeasured inference occupancy.",
        also=[("src/amoeba/turn_api.py",
               '                    }, operation_id=(row["operation_id"] if row is not None',
               "                    }, operation_id=(None if row is not None  # MUTANT: untagged"),
              ("src/amoeba/prompt_api.py",
               '               " harness_constraints, created_at"',
               '               " created_at"  # MUTANT: constraints omitted')],
    ),
    Mutation(
        "I122", "A rebuilt context is one the conversation could have reached by itself",
        "src/amoeba/reconstitution.py",
        "    if at < 0 or ti < 0 or at > ti:\n        return text, False\n",
        "    if True:  # MUTANT: keep the environment and its references\n        return text, False\n",
        ...
        layer="reconstitution / ContextHomeostasis._rebuild (what a new session is made of)",
        note="The primary mutant leaves the dangling references that made the old trim incoherent. Separately verified to die for owed results left oversized, carried turns left unplaced, the governed prompt not reconstructed, an empty generation prompt carried, newest-first removal, and the turn boundary asking for another mode.",
        also=[("src/amoeba/homeostasis.py",
               '        if not chosen["reached_target"]:\n            total = chosen["kept_tokens"]\n',
               '        if False:  # MUTANT: leave owed results oversized\n            total = chosen["kept_tokens"]\n'),
              ("src/amoeba/homeostasis.py",
               '        self._record_carried_spans(new["session_id"], plan.get("placed") or [])\n',
               "        pass  # MUTANT: carried turns become unknown\n"),
              ("src/amoeba/turn_api.py",
               '                mode="rebuild")',
               '                mode="exact")  # MUTANT: the old path'),
              ("src/amoeba/reconstitution.py",
               '        elif any(s.get("lineage") in owed or s["turn_id"] in active for s in held):',
               '        elif any(s.get("lineage") in owed for s in held):  # MUTANT: a continued heartbeat is fair game')],
    ),
    Mutation(
        "I123", "A tool result too large for its delivery budget is never chopped",
        "src/amoeba/roles.py",
        '                    "result_text": out.get("result_text")}',
        '                    "result_text": None}  # MUTANT: drop the bounded text',
        ...
        layer="RoleProcess._invoke / role_tool_invoke / result_read (what enters a role's session)",
        note="The primary mutant is the live defect: every role result fed whole. Separately verified to die for items not kept whole, a reference any role can open, the role's budget ignored, and both prompt views full by default.",
        also=[("src/amoeba/results.py",
              ...
              ("src/amoeba/prompt_api.py",
               '        if detail == "summary":\n',
               '        if detail == "never":  # MUTANT: full by default\n')],
    ),
    Mutation(
        "I124", "Generated tokens may never introduce structural tokens into session state",
        "src/amoeba/backends/llama_engine.py",
        "                if tok in self.structural_tokens:\n                    # Decided on the id",
        "                if False:  # MUTANT: the live forgery\n                    # Decided on the id",
        ...
        layer="LlamaEngine decode loops / DeterministicBackend.generate (what a generation may append)",
        note="The primary mutant is the live defect: Id opened a user message and wrote itself a stale tool result. Separately verified to die for the batched path, for the simulator, for a guard that reads decoded text instead of the token id -- which cannot see the marker at all, because special=False renders it as \"\" -- and for a refusal that leaves no trace.",
        also=[("src/amoeba/backends/llama_engine.py",
               '                    if tok in self.structural_tokens:\n                        a["finish"] = STRUCTURAL_FINISH',
               '                    if False:  # MUTANT: batched is the way in\n                        a["finish"] = STRUCTURAL_FINISH'),
              ("src/amoeba/backends/deterministic.py",
               "        tokens, forged = cut_at_structural(self.tokenize(text),\n                                           self.structural_tokens)",
               "        tokens, forged = list(self.tokenize(text)), None  # MUTANT"),
              ("src/amoeba/mailbox.py",
    ...
    Mutation(
        ...
        also=[("src/amoeba/mailbox.py",
               ...
               'FAILED_STOPS = ("role_failure", "backend_error", "context_pressure")  # MUTANT'),
              ("src/amoeba/supervisor.py",
               "            if len(repairs) >= allowed:",
               "            if False:  # MUTANT: restart it forever"),
              ("src/amoeba/pulse.py",
               '    EventKind.ROLE_NOT_THINKING: "role_not_thinking",\n',
               "")],
    ),
    Mutation(
        "I127", "A count comes with a way to reach the thing counted",
        "src/amoeba/heartbeat.py",
        ...
        note="The primary mutant is the state that made Id invent identifiers: a count of seventeen with nothing to call. Separately verified to die for a dossier that cannot be reached without an id, for a refusal that says nothing about the value, for a merely absent identifier called malformed, and for nothing saying where a real one comes from. Two more come from the same failure one layer along, found live on 2026-09-25 when Ego could not post to its own blackboard: a missing key reported as a bad value, which sent it cycling through relation names it had never got wrong, and a `to_post` that does not exist reaching the FOREIGN KEY constraint, so the reason its post was refused was \"IntegrityError: FOREIGN KEY constraint failed\". A refusal has to name the mistake that was made.",
        also=[("src/amoeba/supervisor_api.py",
               "        if not conclusion_id and not operation_id:\n            # The question Id actually has",
               "        if False:  # MUTANT: no operation to resolve\n            # The question Id actually has"),
              ("src/amoeba/turn_api.py",
              ...
              ("src/amoeba/store/board_repo.py",
               ...
               "                if False:  # MUTANT: the FK constraint answers instead")],
    ),
    Mutation(
        "I128", "The organism is not blind when it is strained",
        "src/amoeba/conditions.py",
        ...
        also=[("src/amoeba/conditions.py",
               ...
               "    if True:  # MUTANT: blind when strained\n        return None"),
              ("src/amoeba/conditions.py",
               "    if streaks.get(role, 0) >= failure_turns > 0:\n        return []",
               "    if False:  # MUTANT: ask a mind that cannot answer\n        return []"),
              ("src/amoeba/supervisor.py",
               "            if when is not None and now - when < cooldown:\n                continue",
               "            if False:  # MUTANT: every pass, forever\n                continue")],
    ),
    Mutation(
        "I129", "An answer that exists is delivered, whatever became of whoever was waiting",
        "src/amoeba/io_api.py",
    ...
    Mutation(
        "I130", "What a client sent arrives whole",
        "src/amoeba/supervisor_api.py",
        ...
        also=[("src/amoeba/supervisor_api.py",
               ...
               '                payload={"question": question[:8000],  # MUTANT\n                         "constraints": constraints[:2000],'),
              ("src/amoeba/io_api.py",
               "                    question=text, interaction_id=interaction_id,\n                    attachments=attachments, wait_seconds=patience)",
               "                    question=text, wait_seconds=patience)  # MUTANT")],
    ),
    Mutation(
        "I131", "Starting a new organism does not erase the old one",
        "src/amoeba/__main__.py",
    ...
    Mutation(
        "I137", "A persistent role delegates only into its own neuocyte lineage",
        "src/amoeba/neuocyte.py",
        "    origin = (item.get(\"origin_actor\") or \"\").strip()\n    return origin if origin in WORK_ORIGINS else \"ego\"",
        '    return "id" if item.get("work_class") == "maintenance" else "ego"  # MUTANT',
        ...
        layer="neuocyte.worker_lineage (whose cognition a work item instantiates)",
        note="The primary mutant is the live defect, restored exactly: lineage taken from `work_class`, which is an argument to `ego_request_work`. Observed 2026-09-24 -- Ego asked for maintenance-shaped work and was handed an id.neuocyte@1.1, Id's cognition instantiated by Ego with no Id involvement in the record. Separately verified to die for a worker handed another role's prefix to inherit physically, and for a lineage read from anywhere but the durable row, which is what keeps a retry or an expiry from re-homing the work.",
        also=[("src/amoeba/neuocyte.py",
               "                    inherited_namespace=None if maintenance else lineage)",
               '                    inherited_namespace=None if maintenance else "ego")  # MUTANT'),
              ("src/amoeba/ego_api.py",
               "        if work_class not in EGO_WORK_CLASSES:",
               "        if work_class not in WORK_CLASSES:  # MUTANT: Ego does maintenance itself"),
              ("src/amoeba/supervisor_api.py",
    ...
    Mutation(
        ...
        "src/amoeba/actions.py",
        "    if not operation_id:\n        return []",
        "    if True:  # MUTANT: an answer accounts for nothing\n        return []",
        ...
        layer="actions.effects_of / mailbox answer / role_answer (what grounds a claim that something was done)",
        note="The primary mutant is the state the organism was in on 2026-09-25, when Ego told a client \"a worker has been delegated\" and no ego_request_work existed anywhere in the turn: the answer accounted for nothing, so nothing contradicted it where the client could see. Separately verified to die for hiding any effect nobody has classified, which is the direction that turns a true statement into an apparently unsupported one; for counting a refused call as a thing done, which would have reported Ego's five board_post calls as five postings when four were refusals and one posted; for crediting one actor with another's effects; and for the receipts not reaching the client, which is the same defect with the accounting merely done in private.",
        also=[("src/amoeba/actions.py",
               "        if kind in TURN_BOOKKEEPING:",
               "        if kind not in OBJECT_KEY:  # MUTANT: unlisted effects go unreported"),
              ("src/amoeba/actions.py",
               '        if got.get("accepted"):',
               "        if True:  # MUTANT: a refused call counts as a thing done"),
              ("src/amoeba/actions.py",
               '        " WHERE operation_id = ? AND actor_id = ? ORDER BY seq",',
               '        " WHERE operation_id = ? AND ? IS NOT NULL ORDER BY seq",'),
              ("src/amoeba/supervisor_api.py",
               '                        "effected": record.get("effected") or [],',
               '                        "effected": [],  # MUTANT: receipts stop here')],
    ),
    Mutation(
        "I140", "An interaction cannot settle while work its answer depends on is unfinished",
        "src/amoeba/io_api.py",
        '                    "   AND status NOT IN (\'done\', \'failed\', \'cancelled\')",\n'
        '                    (operation_id,)).fetchall()]',
        "                    \"   AND status = 'no_such_status'\",  # MUTANT: nothing is ever pending\n"
        "                    (operation_id,)).fetchall()]",
        ...
        layer="io_api._publish (whether a reply may call itself final)",
        note="The primary mutant makes nothing ever count as pending, which is the state the organism was in on 2026-09-25: Ego delegated a computation, ran out of tool-turn budget, and the interaction completed 1.1 seconds after its own worker posted the right number -- so the client was told 41,675,000,250 while 41,679,167,500 sat on the blackboard with nowhere to go. Separately verified to die for asking the question before the mutation instead of inside it, which leaves a window in which work admitted between the check and the commit is settled straight over; for treating any one dependency finishing as all of them, so three workers would release on the first; and for a default that lets work escape the wait unless it opts in, when delegating in order to answer is the ordinary case.",
        also=[("src/amoeba/ego_api.py",
               "                blocks_answer=not bool(background))",
               "                blocks_answer=bool(background))  # MUTANT: opt in to waiting")],
    ),
    Mutation(
        "I141", "Finished answer-work makes its interaction eligible to resume",
        "src/amoeba/io_api.py",
        "        resumed = _resume_parked(limit=max(1, int(limit)))",
        "        resumed = []  # MUTANT: parked is parked forever",
        ["test_finished_work_resumes_the_interaction",
         ...
         "test_one_of_three_finishing_does_not_release_it"],
        layer="io_api._resume_parked (the way out of parking)",
        note="Parking without a way out is a nicer hang, so this is the other half of I140 and not an optimisation. The primary mutant never resumes anything. Separately verified to die for a resumption that does not ask for an answer, which wakes Ego to tell it the work finished and leaves the client waiting anyway; and for waking only when the work succeeded, when \"the computation failed\" is an answer and silence is not. Resumption is the reconciler's rather than a waiting thread's, which is what makes it survive a restart -- no test here has a thread holding the interaction. Also verified to die for a re-check that cannot see an outstanding item, which is the transactional gate: it is the only one, because a second gate at scan time would have to agree with it and would mask it. While this was built the enqueue was wrapped in `except Exception: continue`, and a NameError on an undefined constant was swallowed -- every pass then reported nothing to resume while parked interactions stayed parked, so an internal mistake was indistinguishable from an empty queue. There is now no `except` anywhere in that path and a test says so.",
        also=[("src/amoeba/io_api.py",
               "                    expects_answer=True, operation_id=op, lineage=op)",
               "                    expects_answer=False, operation_id=op, lineage=op)"),
              ("src/amoeba/io_api.py",
               '                    "   AND status NOT IN (\'done\', \'failed\', \'cancelled\')"\n'
               '                    " LIMIT 1", (op,)).fetchone()',
               '                    "   AND status NOT IN (\'done\', \'failed\', \'cancelled\')"\n'
               '                    " AND 0 LIMIT 1", (op,)).fetchone()'),
              ("src/amoeba/io_api.py",
              ...
              ("src/amoeba/io_api.py",
               ...
               "        pass  # MUTANT: handed a status, not a result")],
    ),
    Mutation(
        "I142", "A work result's substance is not hidden behind its telemetry",
        "src/amoeba/store/work_repo.py",
        "            item.update(substance_of(item.get(\"result\")))",
        "            pass  # MUTANT: the finding stays behind the provenance",
        ["test_get_work_surfaces_what_the_work_concluded",
         "test_the_resumed_request_carries_what_the_work_found"],
        layer="work_repo.get_work / results.substance_of (reaching what work concluded)",
        note="A neuocyte's result carries its finding beside its telemetry -- raw text, tool calls, tools offered, posts seen, token counts -- and bounded projection treats the dict alike, so the short part ends up behind the same reference as the long part. Live on 2026-09-25 a worker recorded 41679167500 in `finding` and completed; Ego called `get_work`, then `result_read` three times, never reached it, and told the client the system had failed to deliver the result. It had -- into 2118 bytes of provenance four hops away. Separately verified to die for a substance extractor that finds nothing to lift.",
        also=[("src/amoeba/results.py",
               '            out["finding"] = value.strip()[:MAX_SUBSTANCE]',
               "            pass  # MUTANT: nothing is ever substantive")],
    ),
    Mutation(

...
| An answer is the whole interaction's | **Implemented, tested** | every bounded turn's piece, in order, exactly once, assembled along parent links; a continuation resumes the message it continues where the session allows — see I107, I109 |
| Per-role output ceilings | **Implemented, tested** | Ego 3072, Id 1024, `ego.neuocyte` 512, `id.neuocyte` 384 from governed profiles, with a fallback held equal to the shipped headers — see I110 |
